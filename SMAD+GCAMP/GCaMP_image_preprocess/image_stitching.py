...
image_dir = 'path/to/gcamp/images/'
image_names = os.listdir(image_dir)
image_names.sort()
file_names = [f"path/to/gcamp/images/{s}" for s in image_names]
...
#subtract pixel medians all images
for image_name in image_names:
    file_name = 'path/to/gcamp/images/' + image_name
    image = imageio.imread(file_name)
    image = np.clip(np.subtract(image, median_image), 0, None)
...
image_names = os.listdir(image_dir)
image_names.sort()
time_points_num = 40 #the number of time points
time_points = [f"Z{str(i).zfill(3)}" for i in range(1, time_points_num+1)]

overlap_rate = 0.01 #ovelap rate of gcamp images
col_num = 8 #number of columns of for image stitching
row_num = 17 #number of rows of for image stitching
...
for time_point in time_points:
    ...
    result_image_file_path = 'stitched_images_gcamp/' + time_point + '.npy'
